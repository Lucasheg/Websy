...
class ErrorBoundary extends React.Component {
  ...
  render() {
    if (this.state.error) {
      return (
        <div style={{background:"#fee2e2",border:"1px solid #fecaca",color:"#991b1b",borderRadius:12,padding:16}}>
          <div style={{ fontWeight: 700, marginBottom: 8 }}>A section failed to render</div>
...
export default function App() {
  ...
  // One click delegate for all [data-action] in preview
  ...
  // Generate DSL (fallback to a neutral example site)
  const [dslError, setDslError] = useState(null);
  const dsl = useMemo(() => {
    try {
      setDslError(null);
      const out = generateDSL(maker);
      if (!out || !Array.isArray(out?.pages)) throw new Error("Bad DSL structure");
      return out;
    } catch (e) {
  ...
  return (
    <div style={{ background: "#f6f7f9", minHeight: "100vh" }}>
      ...
      >
        {dslError ? (
          <div style={{background:"#fee2e2",border:"1px solid #fecaca",color:"#991b1b",borderRadius:12,padding:16}}>
            <div style={{ fontWeight: 700, marginBottom: 8 }}>Preview failed to render</div>
